...
import {
  AutoCompletarV2, CardHeaderName, MenuPJuridica, SaveButton, Checkbox, PageTitle,
} from '../../../../components';
import { getPJuridicaRServicoFicha, savePJuridicaRServico, getPJuridicaComercialFicha } from '../../../../functions/cadastro/pessoa-juridica';
...
function FichaServico(props) {
  ...
  const [pag_criterio, setPag_criterio] = useState('');
  const [dpag_criterio, setDpag_criterio] = useState('');

  const [rec_prazo, setRec_prazo] = useState(0);
  const [rec_criterio, setRec_criterio] = useState('');
  ...
  useEffect(() => {
    if (firstLoad) {
      ...
      getPJuridicaRServicoFicha(props, ID_RServico);
...
-
                ...
                {/*** SAVE ***/}
                <SaveButton save={() => savePJuridicaRServico(props, form)} />
              </Row>
            </CardBody>
          </Card>
        </Col>
...
const mapState = (state) => ({
  rservicoFichaData: state.pJuridica.rservicoFichaData,
