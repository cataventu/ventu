...
import { saveMoedaFiltro } from '../../../functions/financeiro/moeda';

function MoedaFiltro(props) {
  ...
  useEffect(() => {
    if (firstLoad && localStorage.getItem('TABELAS_MOEDA_FILTRO')) {
      const filtro = JSON.parse(localStorage.getItem('TABELAS_MOEDA_FILTRO'));
      const {
        descricao, padrao, situacao, alt_dhsis_maior, alt_dhsis_menor,
      } = filtro;
  ...
  const ActionButtons = [
    <Buttons description="Filtrar" color="primary" onClick={() => saveMoedaFiltro(props, form)} title="Salva todas as configurações do filtro" />,
    <Buttons description="Limpar" color="filtro-interno" onClick={() => resetFields()} title="Limpa todas as configurações do filtro" />,
    <Buttons description="Fechar" color="filtro-interno" onClick={() => toggleFiltro(props, 'tabela-moeda-filtro')} title="Limpa todas as configurações do filtro" />,
  ];

  ///////// RENDER FILTRO /////////
  /////////////////////////////////
  return (
    <Container id="tabela-moeda-filtro" fluid className="p-0 filter oculta">
...
export default connect()(MoedaFiltro);
