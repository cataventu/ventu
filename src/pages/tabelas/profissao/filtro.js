...
import { saveProfissaoFiltro } from '../../../functions/tabelas/profissao';

///////// FILTRO PROFISSAO //////////
/////////////////////////////////
function ProfissaoFiltro(props) {
  ...
  useEffect(() => {
    if (firstLoad && localStorage.getItem('TABELAS_PROFISSAO_FILTRO')) {
      const filtro = JSON.parse(localStorage.getItem('TABELAS_PROFISSAO_FILTRO'));
      const {
  ...
  const ActionButtons = [
    <Buttons description="Filtrar" color="primary" onClick={() => saveProfissaoFiltro(props, form)} title="Salva todas as configurações do filtro" />,
    <Buttons description="Limpar" color="filtro-interno" onClick={() => resetFields()} title="Limpa todas as configurações do filtro" />,
    <Buttons description="Fechar" color="filtro-interno" onClick={() => toggleFiltro(props, 'tabela-profissao-filtro')} title="Limpa todas as configurações do filtro" />,
  ];
  ///////// RENDER FILTRO /////////
  /////////////////////////////////
  return (
    <Container id="tabela-profissao-filtro" fluid className="p-0 filter oculta">
...
export default connect()(ProfissaoFiltro);
