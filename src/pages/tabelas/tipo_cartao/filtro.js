...
import { saveCartaoFiltro } from '../../../functions/tabelas/tipo_cartao';

///////// FILTRO CARTAO //////////
/////////////////////////////////
function CartaoFiltro(props) {
  ...
  useEffect(() => {
    if (firstLoad && localStorage.getItem('TABELAS_CARTAO_FILTRO')) {
      const filtro = JSON.parse(localStorage.getItem('TABELAS_CARTAO_FILTRO'));
      const {
  ...
  const ActionButtons = [
    <Buttons description="Filtrar" color="primary" onClick={() => saveCartaoFiltro(props, form)} title="Salva todas as configurações do filtro" />,
    <Buttons description="Limpar" color="filtro-interno" onClick={() => resetFields()} title="Limpa todas as configurações do filtro" />,
    <Buttons description="Fechar" color="filtro-interno" onClick={() => toggleFiltro(props, 'tabela-cartao-filtro')} title="Limpa todas as configurações do filtro" />,
  ];

  ///////// RENDER FILTRO /////////
  /////////////////////////////////
  return (
    <Container id="tabela-cartao-filtro" fluid className="p-0 filter oculta">
...
export default connect()(CartaoFiltro);
